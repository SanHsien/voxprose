...
| `paths.py` | `APP_DATA_DIR`（`%APPDATA%\VoxProse`）、`SYNC_BASE_DIR`（可指標重定向的雲端同步目錄，預設 `Documents\VoxProse_Sync`）、版本號 `VERSION_NAME`/`BUILD_ID`、`initialize_paths()`（建目錄、舊版資料搬遷、真可攜版 `bundled_models/` 首次安裝） |
