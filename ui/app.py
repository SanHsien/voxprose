...
class VoiceTypeApp(QObject):
    ...
    def __init__(self):
        ...
        # docs/DECISIONS.md 的決策理由；REVIEW.md 風險排序表 #10）
        self._recording_mutex = PttVadMutex()
    ...
    def _on_start(self, mode="ptt"):
        ...
        # PTT/切換模式優先權高於 VAD 全時模式：若 VAD 正在擷取一段語音，
        # 直接捨棄該段落再開始 PTT 錄音，避免兩路錄音疊加。
        if self._recording_mutex.on_ptt_start() and self.auto_trigger:
    ...
    def _on_stop(self, mode="ptt"):
        self._recording_mutex.on_ptt_stop()
        if not self._models_ready:
            self.indicator.hide()
            self.tray.set_icon("🎙")
            return
    ...
    def _on_auto_segment_start(self):
        # PTT/切換模式正在錄音時，這次 VAD 觸發直接忽略——不疊加第二路
        # 錄音/UI 狀態；對應的音訊在 _on_auto_segment_stop 也會一併丟棄。
    ...
    def _on_auto_segment_stop(self, wav_bytes):
        if self._recording_mutex.on_vad_segment_stop():
            return
        self._ensure_segment_worker()
        self._segment_queue.put(wav_bytes)

    def _on_audio_complete(self, audio_data):
        # 7-4（Mac 主線 v2.9.7，比照 51094bf:main.py:357）：整段錄音峰值 RMS
        # 低於門檻（recorder.is_silent，見 audio/recorder.py:stop()）直接跳過
        # STT，省一次呼叫並與幻覺過濾互補。只涵蓋 PTT/toggle 路徑——VAD 全時
        # 模式（audio/auto_trigger.py）走獨立的 AutoTriggerController stream，
        # 不共用這個 recorder 實例，其靜音判斷另由 min_speech_sec 把關。
        if self.recorder.is_silent:
            log.info("[process] Recording is silent (below threshold), skipping STT.")
    ...
    def _process_audio(self, audio_data):
        try:
            ...
            # --- 0.4. Whisper 幻覺過濾（對本地與雲端引擎都生效）---
            # 統一路徑：不管 text 是 stt/subprocess_whisper.py（本地）還是
            # groq_whisper/openrouter_stt/gemini_stt（雲端 API）算出來的，都在
            # 這裡（主行程、拿到轉錄結果之後）過濾一次，避免把過濾邏輯塞進
            # subprocess_whisper.py 的子行程而增加 IPC 訊息格式的複雜度。
            try:
                from stt.hallucination_filter import is_hallucination
                if is_hallucination(text):
                    log.info(f"[process] Hallucination filtered: {text!r}")
                    self.ui_signal.emit({"type": "hide", "value": None})
                    return
            except Exception as e:
            ...
            # --- 2.5. LLM 未啟用時套用輕量版靈魂規則（Mac 主線 7-5 移植）---
            if not is_llm_used:
                output_content = self._apply_basic_soul_rules(output_content)
    ...
    def _apply_basic_soul_rules(self, text: str) -> str:
        """Mac 主線 v2.9.7（7-5，`51094bf:main.py:709-745`）移植：LLM 未啟用（或
        showcase/action/demo 都沒觸發）時，STT 原文直出，完全沒有贅詞清理。這裡補上
        一個不需要呼叫任何 LLM API 的輕量版清理——動態讀取 soul md（底層靈魂 + 當前
        情境檔）的「贅詞清除規則」區段，抽出引號內的贅詞逐一刪除。詞語清單完全由
        使用者在 soul 檔裡自行維護，不 hardcode 在程式碼中。實際的字串解析/刪除邏輯
        在 utils/soul_rules.py（純函式，供單元測試），這裡只負責讀檔。"""
        ...
        for path in (SOUL_BASE_PATH, scenario_path):
            try:
                ...
                log.error(f"[process] basic_soul_rules read error ({path}): {e}")

        try:
            return apply_basic_soul_rules(text, contents)
        except Exception as e:
            log.error(f"[process] basic_soul_rules filler error: {e}")
            return text.strip()
    ...
    def _on_config_saved(self, new_config=None):
        with self._config_lock:
            ...
            self.stt = get_stt(self.config)
            self._apply_auto_trigger()
            # Mac 主線 7-1/7-2/7-3：麥克風裝置/增益/AGC 設定變更即時套用
            # （不重建 recorder 物件，避免打斷既有 on_start/on_stop 接線；
            # 下一次 start() 才會用新的 device 開新 stream，屬預期行為）
