...
## 2026-08-29（補）— 依賴新鮮度加上「已核准暫緩」機制

- **問題**：anthropic 已判斷過、明確決定維持 `<1`（見上一條），但檢查器只認「有沒有超出上限」，不認「有沒有判斷過」。結果 issue #1 會每月穩定復發一次，內容永遠是同一筆已經有結論的東西。**每月一次的假警報比沒有警報更糟**——它會訓練人略過這份報告。
- **決定**：比照 voxavatar／agentdeck／gpt-ai-assistant，加上 `.github/dependency-deferrals.json`。暫緩必須寫理由，而且**綁定當初判斷的那個版本**（`deferredLatest`）：上游一發更新的版本，暫緩自動失效、該列重新浮出來要求再看一次。
- **考慮過的替代方案**：把 anthropic 的上限直接開到 `<2`（等於假裝已經升級）、或在 workflow 加 ignore list（理由不會留在版控裡）。兩者都是用「看不見」換「沒問題」。
- **驗證**：新增 5 條測試——暫緩對判斷過的版本生效且 `blocked_by_upper` 仍如實為 `True`（報告要看得出它超出上限，只是判斷過）、上游發新版時暫緩失效、沒寫理由的暫緩不生效、檔案缺失或壞掉時一律視為沒有暫緩、以及 repo 內實際的暫緩檔每筆都有版本與理由。486 passed。
