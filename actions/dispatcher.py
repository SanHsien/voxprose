...
class ActionDispatcher:
    ...
    def dispatch(self, text: str, target_bundle_id: str = "", target_pid: int = 0) -> bool:
        """
        ...
        if re.search(r"天氣(如何|怎麼樣|好不好)?$", text) or "查天氣" in text:
            result = get_weather()
            self._finish_action(result, target_bundle_id, target_pid)
            return True
        ...
        if switch_match:
            ...
            self._finish_action(msg, target_bundle_id, target_pid)
            return True
    ...
    def _finish_action(self, msg: str, target_bundle_id: str = "", target_pid: int = 0):
        """執行完動作後的統一回饋。"""
        self.indicator.flash()
        self.indicator.set_state("done")
        # 語音指令的回應通常也直接注入到目前輸入框，或是僅在 Dashboard 顯示
        # 這裡設定直接注入，方便使用者直接得到答案
        self.injector.inject(f"「{msg}」", target_bundle_id, target_pid)
